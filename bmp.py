"""A module for dealing with BMP bitmap image file"""
...
def write_grayscale(filename, pixels):
    ...
    with open(filename, 'wb') as bmp:
        ...
        size_bookmark = bmp.tell() # The next four bytes hold the filesize as 32bit
        bmp.write(b'\x00\x00\x00\x00') # little endian integer. Zero placeholder for now

        bmp.write(b'\x00\x00') # Unused 16bit integer - should be zero
        bmp.write(b'\x00\x00') # Unused 16bit integer - should be zero
...
def _int32_to_bytes(i):
    """Convert an integer to four bytes in little_endian format"""
    return bytes((i & 0xff,
                  i >> 8 & 0xff,
                  i >> 16 & 0xff,
                  i >> 24 & 0xff))
...
def dimensions(filename):
    ...
    Raises:
        ValueError: If the file was not BMP
